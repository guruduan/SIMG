...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && confirm_sesskey() && $action === 'save') {
    ...
    if (!$kelasid || !$userid) {
        redirect(new moodle_url('/local/jurnalmengajar/wali_kelas.php'), '⚠️ Data tidak valid', null, \core\output\notification::NOTIFY_ERROR);
    }

    $mapping[$kelasid] = $userid;
    set_config('wali_kelas_mapping', json_encode($mapping), 'local_jurnalmengajar');
    redirect(new moodle_url('/local/jurnalmengajar/wali_kelas.php'), '✅ Mapping berhasil disimpan', null, \core\output\notification::NOTIFY_SUCCESS);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && confirm_sesskey() && $action === 'delete') {
    ...
    if (isset($mapping[$deleteid])) {
        ...
        set_config('wali_kelas_mapping', json_encode($mapping), 'local_jurnalmengajar');
        redirect(new moodle_url('/local/jurnalmengajar/wali_kelas.php'), '🗑️ Mapping berhasil dihapus', null, \core\output\notification::NOTIFY_SUCCESS);
    }
}
