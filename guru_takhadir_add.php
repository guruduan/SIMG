...
$PAGE->set_url('/local/jurnalmengajar/guru_takhadir_add.php');
$PAGE->set_title('Tambah Guru Tidak Hadir');
$PAGE->set_heading('Tambah Guru Tidak Hadir');

$userid         = optional_param('userid', 0, PARAM_INT);
$status         = optional_param('status', '', PARAM_ALPHA);
$tanggalmulai   = optional_param('tanggalmulai', date('Y-m-d'), PARAM_TEXT);
$tanggalselesai = optional_param('tanggalselesai', date('Y-m-d'), PARAM_TEXT);
...
if (optional_param('save', 0, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (empty($userid)) {
        ...
        if ($DB->record_exists_sql($sql, $params)) {
            $error = 'Guru tersebut sudah memiliki data pada rentang tanggal tersebut.';
        }
    }
    ...
    if (empty($error)) {
        ...
        $DB->insert_record('local_jurnalmengajar_kehadiran', $record);

        redirect(
            new moodle_url('/local/jurnalmengajar/guru_takhadir.php'),
            'Data guru tidak hadir berhasil disimpan.',
            null,
...
//echo $OUTPUT->heading('Tambah Guru Tidak Hadir');
